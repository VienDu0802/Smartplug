...
#define HLW8032_GPIO  GPIO_NUM_25
...
void task_hlw8032_rcv(void* arg)
{
    ...
    while (1)
    {
        ...
        {
            float voltage = hlw8032_get_V(&hlw8032);
            float current = hlw8032_get_I(&hlw8032);
            float power = hlw8032_get_P_active(&hlw8032);

            ESP_LOGI(__func__, "Voltage: %.3f V", voltage);
            ESP_LOGI(__func__, "Current: %.3f A", current);
            ESP_LOGI(__func__, "Power: %.3f W", power);

            // Tính toán tổng năng lượng tiêu thụ (kWh)
            total_energy_kwh += (power / 1000.0) * (1.0 / 3600.0); // Cập nhật mỗi giây
            ESP_LOGI(__func__, "Total Energy: %.6f kWh", total_energy_kwh);
        }
        else if (err != ESP_OK)
        {
...
void hlw8032App(){
    ...
    xTaskCreatePinnedToCore(&task_hlw8032_rcv, "hlw8032_rcv", HLW8032_APP_TASK_STACK_SIZE, NULL, HLW8032_APP_TASK_PRIORITY, NULL, HLW8032_APP_TASK_CORE_ID);
}